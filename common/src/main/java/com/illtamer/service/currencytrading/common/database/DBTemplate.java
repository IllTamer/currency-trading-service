package com.illtamer.service.currencytrading.common.database;
...
public class DBTemplate {
    ...
    @Autowired
    public DBTemplate(JdbcTemplate jdbcTemplate) {
        this.jdbcTemplate = jdbcTemplate;
        final String packageName = getClass().getPackageName();
        int pos = packageName.lastIndexOf('.');
        String basePackage = packageName.substring(0, pos) + ".model";
        ...
        try {
            ...
            log.error("Database template create exception", e);
        }
    ...
    <T> void doInsert(T bean, boolean isIgnore) {
        try {
            ...
            if (log.isDebugEnabled()) {
                log.debug("SQL: {}", isIgnore ? mapper.insertIgnoreSQL : mapper.insertSQL);
            }
