...
final class Criteria<T> {
    ...
    List<T> list() {
        ...
        return db.getJdbcTemplate().query(selectSql, mapper.resultSetExtractor, selectParams);
    }
    ...
    T first() {
        ...
        String selectSql = sql();
        Object[] selectParams = params();
        List<T> list = db.getJdbcTemplate().query(selectSql, mapper.resultSetExtractor, selectParams);
        if (list.isEmpty()) {
            return null;
        }
        return list.get(0);
